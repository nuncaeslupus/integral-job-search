...
fenced by a marker ending in `0428fabc771a70448a6b6073`, minted for this packet
...
----- BEGIN INTENT 0428fabc771a70448a6b6073 -----
---
id: t-9d41c7f5
...
----- END INTENT 0428fabc771a70448a6b6073 -----

## The change

- Base commit: `f2633eac834698f73492c39a1d2bf0780c35e746`
- Diff digest: `0a1749843e732e483b06bc14f69e608a17671f16fdd08475bbfa71d79bef14f7`
...
```
 ...
 (untracked) arsenal/tasks/t-7b3ac419.md
...
----- BEGIN DIFF 0428fabc771a70448a6b6073 -----
diff --git a/arsenal/tasks/t-7b3ac419.md b/arsenal/tasks/t-7b3ac419.md
new file mode 100644
index 0000000..14edb26
--- /dev/null
+++ b/arsenal/tasks/t-7b3ac419.md
@@ -0,0 +1,81 @@
+---
+id: t-7b3ac419
...
index 0c6e85e52bee7f81b21b40af4bf65d8dd4611a4f..064f3e6018fee0821933e869922f1a6489c721e8 100644
...
index d0b3fc82843b5a7c53c18a75f09f1c29394a9c97..8c5b82a075cc0aaf5538de8d51608b621acb0a54 100644
--- a/status/plan.md
+++ b/status/plan.md
...
@@ -418,6 +418,7 @@ plan is a complete ledger of the queue rather than of the implementation only.
 | T148 | Two silent corruptions in one letter in one session, neither of which failed anything. Paragraphs edited **by index** after an earlier edit had removed one: `text[4]` and `text[8]` overwrote the leap-of-faith and honest-gaps paragraphs with duplicates of two others, and the write, the render and the paragraph count were all correct. A filter written `if b.get("text") is not None` dropped the letter's `to` block, so the letter was addressed to nobody. Both were found by running `pdftotext` and reading, which is not a check — `integral.ats.check_document` passed both times, correctly, because the text layer was intact and simply said the wrong thing | 5 | M | T142 | `document_edit_defects == 0` | An edit names the blocks it changes and every block it does not name comes out **byte-identical in the rendered text layer**; block count is preserved unless the edit says it changes. The document-level analogue of what `tools/verified_gate.sh` enforces for code, for the same reason: what matters is the artefact that ships, not that the operation returned success. The measurement replays both defects and requires each refused, **plus one legitimate edit that must be allowed** — a gate refusing every edit would score zero and be useless | ☐ |
 | T149 | The tool has been run once from the first question to a sent application — identify through document generation, thirteen revisions, only step 12 untouched — and none of it is a fixture, so a step skill can be changed and nothing notices until a live session goes wrong. Every gate in the repository measures one step; nothing replays the sequence | 5 | M | — | `end_to_end_replay_defects == 0` | A **translation, not a copy**: the run lives in one real person's tree under `~/.integral-job-search/` and stays there, while its *shape* becomes a fixture over the stated fictional candidate `tests/fixtures/generation/master.json` already provides — the step sequence including the steps revisited, the artefact each produced, and what each document revision changed. Measured at the level where the failures actually happened: not "does this function return" but "did step 9 hand step 10 something step 10 could use". Interview preparation is out of scope until it has been run once — there is nothing to freeze | ☐ |
 | T150 | `status/evidence/T125.json` commits `files_checked` as an exact value and it counts every file `ruff format` touches, which since ruff 0.16 includes **Markdown** — `arsenal/tasks/` alone contributes 189 of it. Met on the pull request that seeded T141–T149: nine task files, no Python touched, and `make evidence` went red with `453 -> 462`. Committing the new value is not a fix, because the next task PR moves it again; a gate that goes red for a reason unrelated to what it measures teaches sessions that red is normal | 10 | S | — | `unstable_evidence_keys == 0` | T100's finding in a key T100 did not reach: `archive_sensitive_evidence_keys` compares the committed record across a simulated **archive**, and adding a file is not archiving one. Two pieces, the second being the point — `files_checked` becomes the floor `files_checked_at_least` asserted against a constant as `naming.MINIMUM_SCANNED` already is, and the sensitivity check widens so **every** evidence key is compared across adding a task file as well as archiving one. Otherwise the third instance is found the way the second was, by a pull request going red for no reason | ☐ |
+| T151 | `integral.robots` — the matcher `CLAUDE.md` tells every session to adjudicate a board with — answers **ALLOW** for `/foo/bar?baz=https://foo.bar` under `Disallow: /foo/bar?baz=https%3A%2F%2Ffoo.bar`. RFC 9309 §2.2.2's own example table gives that request the path-to-match `/foo/bar?baz=https%3A%2F%2Ffoo.bar` — reserved octets used as **data** in a query are percent-encoded before comparison while the query's own delimiters are not — so the rule refuses it. The cause is one constant: `_CHUNK_SAFE = "/:@!&'()+,;=?~-._"` leaves `:` and `/` unencoded everywhere, so rule and request never compare equal. Fail-open, in the matcher that decides real fetches. Found by running T120's independent 49-case table against the primary matcher as well as the new second reader — one disagreement out of 49. T120 committed the proof and a pin (`REPO_MATCHER_DISAGREEMENTS`, `repo_matcher_verdicts_against_the_rfc: 1`) but could not fix the primary matcher inside its own scope; a pin with no expiry is a permanent green test asserting a known fail-open is still there, which is what this task ends. Derive the fix from the RFC, **not** by copying `second_reader.canonical` — two matchers derived from one reading are one matcher with two names | 10 | S | T120 | `repo_matcher_verdicts_against_the_rfc == 0` | The metric already exists and already reads 1: T120 wrote it with nobody assigned to it, and this is the assignment. T70's `robots_verdicts_misread` must stay 0 across the change — percent-encoding equivalence is the area that took ten defects across five review rounds | ☐ |
...
----- END DIFF 0428fabc771a70448a6b6073 -----
