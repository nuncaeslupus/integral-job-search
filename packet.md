...
fenced by a marker ending in `6b94aed5cd98e4c328f60b31`, minted for this packet
...
----- BEGIN INTENT 6b94aed5cd98e4c328f60b31 -----
---
id: t-9d41c7f5
...
----- END INTENT 6b94aed5cd98e4c328f60b31 -----

## The change

- Base commit: `1e2e6c82a0858ffc6cab02ff94e032c4c67bddc7`
- Diff digest: `da2546b04c61685418bdb1a51f9eb3789796748a2bf06cff08bd3e0a98677d5f`
- Regenerate in full: `git diff 1e2e6c82a085` (plus untracked files below)

Files touched:

```
 src/integral/connector_policy.py | 212 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++------
 status/evidence/T116.json        |   2 +-
 status/evidence/T120.json        |   8 ++++++
 status/evidence/T125.json        |   2 +-
 status/evidence/T85.json         |   4 +--
 status/plan.md                   |   2 +-
 tests/test_connector_policy.py   |  98 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++----------
 7 files changed, 302 insertions(+), 26 deletions(-)
...
----- BEGIN DIFF 6b94aed5cd98e4c328f60b31 -----
...
+            in_query = True
+            out.append(char)
+        elif ord(char) > 127 or (in_query and char in _QUERY_DATA_OCTETS):
...
+    r"""§2.2.3's two metacharacters, compiled.
+
+    `*` is 'any sequence of characters', explicitly **including the empty
+    sequence**, so it becomes `.*` and never `.+`. `$` 'designates the end of
+    the match pattern', so a **trailing** `$` becomes an end anchor.
+
+    A `$` anywhere else is an ordinary octet. §2.2.3's sentence describes a
+    character at the end of a pattern and says nothing about one in the middle,
+    so two readings survive it: that `$` is special only in final position, or
+    that it always anchors. The second makes `Disallow: /a$b` describe a path
+    that ends and then continues — which no path does — so the rule would match
+    nothing and `/a$b` would be **allowed**, in a file that plainly disallows
+    it. The first is the reading under which the operator gets what they wrote,
+    and it is the fail-closed one of the two; that decides it here. (This module
+    took the anchoring reading first, and the independent case table caught it:
+    `dollar_in_middle_of_pattern`, marked LOW confidence and fail-open.)
+
+    Everything else is a literal, `re.escape`d, so a pattern carrying `.` or
+    `+` or `(` matches those characters and not their regex meaning.
+
+    The match is anchored at the **start** of the request target and is a prefix
+    match: §2.2.2's rules match the beginning of the path, which is what makes
+    `Disallow: /admin` cover `/admin/users`.
+    """
+    out = ["\\A"]
+    last = len(pattern) - 1
...
+    """Does one rule pattern cover this request target?
+
+    An **empty** pattern matches nothing. §2.2.2's `Disallow:` with no value is
+    the documented way to restrict nothing at all, and it is reached here rather
+    than by a special case: an empty pattern has no octets, so it cannot be the
+    most specific match, and treating it as the zero-length prefix that matches
+    every path would turn the one line meaning 'everything is allowed' into a
+    rule covering every path in the file.
+    """
+    if not pattern:
+        return False
+    return _to_regex(pattern).match(target) is not None
+
+
...
+    best: Rule | None = None
+    for rule in rules:
+        if not matches(rule.pattern, wanted):
+            continue
...
+        direction=FAIL_OPEN_RISK,
+        confidence="HIGH",
+        confidence_note="## B. Anchoring (§2.2.2)",
+    ),
+    # ---- 10. prefix_match_crosses_segment_boundary -----------------------
+    Case(
...
+            "defect to fix blind. --- ## D. Where the specificity metric is ambiguous (§2.2.2) "
+            "Every row here exists to make the (P)/(M) choice from the conventions section "
+            "*visible*."
+        ),
+    ),
+    # ---- 23. wildcard_specificity_readings_agree -------------------------
...
+            "exactly what the RFC leaves open. --- ## E. Percent-encoding equivalence (§2.2.2) "
+            "The governing text, recalled: octets in the URI and in robots.txt paths that are "
+            "outside the US-ASCII range, or in RFC 3986's reserved range, MUST be "
+            "percent-encoded before comparison — and the §2.2.2 example table shows the "
+            "converse in its last row, `/foo/bar/%62%61%7A` being compared as `/foo/bar/baz`, "
+            "i.e. percent-encoded **unreserved** octets are decoded before comparison. Rule and "
+            "path are compared after that canonicalisation on both sides."
+        ),
+    ),
+    # ---- 26. pct_unreserved_encoded_in_rule ------------------------------
+    Case(
...
+            "deliberately not asserted here. --- ## F. Query strings and case (§2.2.1, §2.2.2)"
+        ),
+    ),
+    # ---- 35. query_is_part_of_the_matched_string -------------------------
+    Case(
...
+            "mirrored file. --- ## G. Groups and user-agent matching (§2.2, §2.2.1)"
+        ),
+    ),
+    # ---- 38. product_token_matching_is_case_insensitive ------------------
+    Case(
...
+            "the ABNF's `*WS`, not a separate quoted sentence. --- ## Using this table Each row "
+            "is a fixture, not a comment. Per CLAUDE.md, an accepted case is only accepted once "
+            "it is **committed into the gate's own fixtures before the PR merges** — a report "
+            "that is read and waved through leaves the code exactly as unprotected as it was, "
+            "and the measured denominator must rise by the number of cases accepted. Where a "
+            "row is `LOW`, the finding is a *reading disagreement* to settle in writing (cases "
+            "22, 24, 25, 46), not automatically a defect. Where a row is `HIGH` and "
+            "`FAIL_OPEN_RISK`, a disagreeing matcher is fetching something the operator "
+            "refused."
+        ),
+    ),
+)
...
index a55b754c1d057685a98984749e2ee665f6ac03f1..6aca302ab6b375af779b32d6b420fdfe31f37181 100644
...
@@ -946,8 +1140,9 @@ class RobotsAdjudication:
         """
...
+            found = _classify(self.robots_txt, self.agent, ask)
         ...
         problems: list[str] = []
@@ -965,7 +1160,7 @@ class RobotsAdjudication:
...
@@ -1246,11 +1441,12 @@ def measure_second_readers(
...
@@ -1261,7 +1457,7 @@ def measure_second_readers(
             )
     violations += [
...
index c9e637d79f28a84ffe9abe3c42915413b06b97cb..49f7b862cf0faf6e7a8cccd406165ea74bda1e97 100644
...
new file mode 100644
index 0000000000000000000000000000000000000000..a486857dce07fe5d38590e79b16c8c6b5fbb97cc
--- /dev/null
...
index 694d57c38c4ffe2b9570b7aeedbd39c8a91c457d..0c6e85e52bee7f81b21b40af4bf65d8dd4611a4f 100644
...
index 3ba32cf337c6d74be4dfd6c11d3cb933a6d917ab..fb54d9dce93527addba8143967b1ad634e8f78f8 100644
--- a/status/evidence/T85.json
+++ b/status/evidence/T85.json
@@ -1,7 +1,7 @@
 {
   "gate_modules_outside_the_evidence_run": 0,
-  "gate_modules_outside_the_evidence_run_evaluated": 98,
-  "gate_modules_discovered": 98,
+  "gate_modules_outside_the_evidence_run_evaluated": 99,
+  "gate_modules_discovered": 99,
   "modules_missing": [],
   "gate_status": "measured"
 }
diff --git a/status/plan.md b/status/plan.md
index 2d4498b41d184b806a1d8d8d9004017e12bf2f62..9cc2e1a4899a981a0d5e699524fbf9363659d42d 100644
--- a/status/plan.md
+++ b/status/plan.md
@@ -385,7 +385,7 @@ plan is a complete ledger of the queue rather than of the implementation only.
...
index 608c9383970537e30224ab538c337954847fd182..99620b8c58b9a5fdf5168ea42bc90ef0c4be5e18 100644
...
@@ -889,3 +896,68 @@ def test_single_parser_names_which_of_its_two_findings_each_row_is() -> None:
...
----- END DIFF 6b94aed5cd98e4c328f60b31 -----
