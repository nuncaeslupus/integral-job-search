...
fenced by a marker ending in `b1687781298597dab182ea08`, minted for this packet
...
----- BEGIN INTENT b1687781298597dab182ea08 -----
---
id: t-9d41c7f5
...
> **Truncated**: 4142 lines, showing the first 4000. The rest is NOT below.
> The file list above is complete even though this diff is not, so treat
> every path listed there as unread until confirmed. Read the missing
> ones directly before reaching a verdict:
>   - tracked:   `git diff f2633eac8346 -- <path>`
>   - untracked (marked `(untracked)` in the list): open the file itself.
>     `git diff` prints NOTHING for a path git does not track, and an
>     empty result there means "not tracked", never "not changed".
> If you do not read them, say so and BLOCK: a verdict on a diff you only
> partly saw is worse than no verdict.

----- BEGIN DIFF b1687781298597dab182ea08 -----
...
+    query admits — encoded, as written, and decoded — and a rule matching any of
+    them matches. Comparing against more spellings can only
+    make more rules apply — it never makes a `Disallow` stop matching — which is
+    the direction to be wrong in if this reading of §2.2.2 is wrong.
+
...
+    best: Rule | None = None
+    for rule in rules:
+        if not any(matches(rule.pattern, spelling) for spelling in wanted):
+            continue
...
index 4a848d7cdc047926ae3246f7f700a88fdbbda880..04f6f22929f047e90b97952371592769af429b7f 100644
...
new file mode 100644
index 0000000000000000000000000000000000000000..bcaca25c0999983047ab3fd1c3e3f8316a12c762
--- /dev/null
...
----- END DIFF b1687781298597dab182ea08 -----
