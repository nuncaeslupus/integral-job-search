...
| T57 | `ontology_hit_rate >= 0.85` once a read pass over a corpus wide enough to fall outside the model is allowed to name what it cannot map. **Split out of T17 2026-08-22** — its gate declares `status-key: ontology_status`, so "no source can yet contradict a 1.0" records as unmeasured rather than as a hard failure | 8 | M | T25, T26 | `ontology_hit_rate >= 0.85` | `test_unmapped_concepts_are_counted_not_discarded` in `tests/test_ontology_health.py`, extended to the widened model | ☐ |
...
| T58 | Consume `claude-arsenal` from the marketplace instead of a vendored subtree. v1.0.0 is a Claude Code marketplace, so the two plugins install into `~/.claude/plugins/cache/` and the repo carries no copy of upstream: `vendor/claude-arsenal`, `verify-subtree`, `arsenal-upgrade`, `update-skills` and `assemble-bundle` all retire, and `host-gate` drops to four targets. **This supersedes S9** — a squash merge drops the `git-subtree-split:` trailer, so every upgrade replayed from a stale base and conflicted on files carrying no local edits, resolved each time by taking upstream verbatim. The assembled bundle under `claude-arsenal/` stays: upstream's own protocol prescribes it, and `init.py` out of the installed plugin refreshes it | all | M | S9 | `vendored_upstream_files == 0` | `test_no_upstream_tree_is_vendored` in `tests/test_arsenal_source.py`; `test_re_adding_a_vendored_tree_is_counted`, so the metric cannot pass by having no teeth; `test_subtree_machinery_coming_back_is_named` | ☐ |
