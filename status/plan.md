...
| T88 | The eligibility requirement patterns are English-only, so a Spanish or Catalan advert stating a bar reads as silence — plus a targetless English bar that matches nothing, and a targetless pattern that drags its own targeted PASS to FLAG | 9 | M | T86 | `bars_read_as_silence == 0` | `test_a_spanish_advert_stating_a_bar_is_not_read_as_silence` in `tests/test_eligibility.py`; `test_a_catalan_advert_stating_a_bar_is_not_read_as_silence`; `test_a_targetless_english_work_permit_bar_is_found`; `test_a_targeted_bar_is_not_dragged_to_flag_by_its_own_targetless_pattern` | ☐ |
