...
| T59 | `extraction_negation_recall >= 0.80` over the corpus subset labelled as negated, once that subset can carry a number. **Split out of T16 2026-08-23** — its gate declares `status-key: negation_status`, so "2 negated labels against a floor of 10" records as unmeasured rather than as a hard failure | 8 | M | T25, T26 | `extraction_negation_recall >= 0.80` | `test_negated_labels_are_recovered` in `tests/test_negation.py` — recall ≥ 0.80 over the negated subset, once the subset can carry one | ☐ |
