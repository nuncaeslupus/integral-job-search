...
| D-26 | A retraction never reaches the finished document, and the finished document is the send. After `retract`, `cv/generated/<offer>/v1/letter.md` still contains the withdrawn story and `payload.json` still lists it: `retraction.purge_derived_citations` scans `{profile, rankings, extractions, annotations}` and never `cv/generated`. §4.1 requires a retracted fact to be suppressed *everywhere derived, in the same turn*, and §6.2 and step 11 both stop one step short of sending — so the file on disk **is** what reaches the employer, and D-24's boundary can only stop the next draft and the send record. Found and reproduced on disk by the #305 second reader. **The purge must skip any version already named in `applications/`**: that record is the only copy of what was actually sent and §7 makes it immutable | 11 | M | D-24, T46 | `retracted_stories_in_generated_documents == 0` | `test_a_retraction_purges_the_generated_letter` in `tests/test_retraction.py`; `test_a_version_already_recorded_as_sent_is_not_purged`; `test_the_purge_reports_unmeasured_when_no_version_carries_a_retracted_story` | ☐ |
...
| **M4 — per opportunity** | documents for one advert, and the interview around it | S4, D-9, T45, T46, S6, T47, T43, T22, T26b, T25, T29, T51, T52, T53, T54, T55, S9, S10, S11, S12, D-24, D-26 |
