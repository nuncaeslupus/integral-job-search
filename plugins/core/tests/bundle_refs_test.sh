...
# `claude-arsenal/bin/<name>.sh` and `claude-arsenal/scripts/<name>.py` named
# anywhere in the bundle or the init skill must be a file the bundle ships.
#
...
for src in "${sources[@]}"; do
    ...
    done < <(grep -oE 'claude-arsenal/(bin|scripts)/[A-Za-z0-9_.-]+\.(sh|py)' "${src}" | sort -u)
done
...
if [[ ${fail} -eq 0 ]]; then
    echo "PASS: bundle_refs_test — every referenced bundle script exists"
fi
exit "${fail}"
