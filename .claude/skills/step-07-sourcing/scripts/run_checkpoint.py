#!/usr/bin/env python3
"""run_checkpoint.py — step 7 (Sourcing) coverage checkpoint (S7).

Reads one candidate's session state (T35) and profile tree (T34) and reports whether
step `sourcing`'s *machine-visible* half of its stop rule is met: every artefact it
produces is present, and the session's recorded position for this step carries nothing
outstanding. See `status/spec-v2-steps.md`, step 7, for the full stop rule —
...
Run via (from the repo root, with the project's dev environment):
    uv run python3 .claude/skills/step-07-sourcing/scripts/run_checkpoint.py \
...
def checkpoint(profiles_root: Path, handle: str) -> dict[str, Any]:
    """This candidate's machine-visible coverage state for `sourcing` — computed, not asserted."""
    steps = load_steps()
