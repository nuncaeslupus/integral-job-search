...
If the runtime (`jobsearch.step_runtime.offered`) is not offering `history` for this candidate, defer to whichever step it does offer instead of running this one out of turn.
...
result to the candidate's own tree at `session/checkpoint-history.json`, never to a shared or
global path.
