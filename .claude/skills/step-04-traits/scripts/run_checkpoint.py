#!/usr/bin/env python3
"""run_checkpoint.py — step 4 (Traits) coverage checkpoint (S7).

Reads one candidate's session state (T35) and profile tree (T34) and reports whether
step `traits`'s *machine-visible* half of its stop rule is met: every artefact it
produces is present, and the session's recorded position for this step carries nothing
outstanding. See `status/spec-v2-steps.md`, step 4, for the full stop rule —
most of it is conversational and cannot be read from a file; this checks only the half
a file can answer.

This is **not** the step's acceptance gate. `trait_evidence_sufficiency == 1.0`,
owned by T28, measures something this script does not attempt, and is
...
Run via (from the repo root, with the project's dev environment):
    uv run python3 .claude/skills/step-04-traits/scripts/run_checkpoint.py \
...
def checkpoint(profiles_root: Path, handle: str) -> dict[str, Any]:
    """This candidate's machine-visible coverage state for `traits` — computed, not asserted."""
    steps = load_steps()
