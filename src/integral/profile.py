...
class EvidenceLog:
    ...
    def effective_rows(self) -> list[EvidenceRow]:
        ...
        return [row for row in self.rows() if row.kind != "retraction" and row.id not in suppressed]
...
def _last_pinned_value(
    ...
    T41's function — `integral.constraints_step` imports this module at load
...
def write_evidence(evidence: Path = DEFAULT_EVIDENCE_PATH) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="integral-t6-") as tmp:
...
def write_constraint_survival_evidence(
    ...
    with tempfile.TemporaryDirectory(prefix="integral-d6-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m integral.profile [path]` → T6's gate evidence.

    `python -m integral.profile --constraint-survival [path]` → D-6's gate
