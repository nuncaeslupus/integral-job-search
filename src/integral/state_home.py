...
`ProfileLeak` already buys for cross-profile reads in `integral.identity`.
...
def store_call_sites(repo_root: Path = _REPO_ROOT) -> list[Path]:
    ...
    The thirteen step checkpoints plus `integral.identity`, which is where the
    ...
    identity = repo_root / "src" / "integral" / "identity.py"
    if identity.is_file():
...
def check_call_site(path: Path, repo_root: Path = _REPO_ROOT) -> SiteCheck:
    ...
    if not resolves:
        reasons.append(
            "does not resolve its store root through integral.state_home "
...
def code_lines(text: str) -> list[tuple[int, str]]:
    ...
    prose. Without this, the paragraph in `integral.identity` explaining which
...
def _main(argv: list[str]) -> int:
    """`python -m integral.state_home [--check] [--write-evidence [PATH]] [--where]`.
