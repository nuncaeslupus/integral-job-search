...
from integral.identity import Identity, ProfileStore
...
def purge_derived_citations(store: ProfileStore) -> list[str]:
    ...
    `rebuild` recomputes the four files `integral.profile` owns. Rankings and
...
def derived_files(store: ProfileStore) -> list[Path]:
    ...
    The gate scans all of them rather than the four `integral.profile` writes:
...
def write_evidence(evidence: Path = DEFAULT_EVIDENCE_PATH) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="integral-t38-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m integral.retraction [path]` → T38's gate evidence."""
