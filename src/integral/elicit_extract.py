...
are extracted against those same IDs." T7 (`integral.question_bank`) built
...
directly (`integral.profile`, T6). So this module owns exactly one thing —
...
`integral.decline.DeclineLedger` guarantees "a subject declined once is not
...
writes. `integral.profile._build_stories` projects `kind="episode"` rows into
`stories.jsonl`; `integral.profile._build_traits` projects every row's
...
def candidate_dimensions(
    ...
    *,
    ...
    hits = [
        (did, negated)
        for did, negated in _matched_cue_hits(answer, dimensions, language=language)
        if did != primary
    ]
...
def probe_extraction(root: Path) -> dict[str, Any]:
    ...
    uses.
    """
    from integral.identity import ProfileStore as _ProfileStore
    ...
    secondary_answer = (
        "They let me pick my own priorities, though the on-call rotation ran every third week."
    )
    ...
    review_answer = (
        "I'd rather not get into that, but there was constant overtime expected of the whole team."
    )
...
def measure() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="integral-t8-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m integral.elicit_extract [--check] [--write-evidence [PATH]]` -> T8's gate.
