...
what "two occasions" means, and does not run the interview. `integral.interview`
...
second copy of the floor, which is the drift `integral.profile`'s own module
...
def measure() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="integral-t49-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m integral.trait_sufficiency [--check] [--write-evidence [PATH]]` -> T49's gate."""
