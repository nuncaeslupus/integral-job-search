...
T7 (`integral.question_bank`) generates questions from the dimension model. T8
(`integral.elicit_extract`) turns one answer into evidence. Neither is an
...
module's own judgement**: T35 (`integral.session`) already builds the whole
...
from integral.elicit_extract import ExtractionResult, store_answer
from integral.profile import EvidenceLog, EvidenceRow
from integral.question_bank import BankEntry, QuestionBank, build_bank
...
def run_interview(
    ...
    *,
    ...
    `integral.session.SessionStore`, in `Position.covered`) and pass back in.
    Nothing here reads a clock beyond `now`, which the caller supplies for the
    same reason `integral.profile` never stamps a derived file from the wall
...
def measure() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="integral-t27-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m integral.interview [--check] [--write-evidence [PATH]]` -> T27's gate."""
