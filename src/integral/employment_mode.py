...
than re-deciding it.** `integral.candidate.EmploymentModeName` is
...
by that shape more than once (see `integral.connector_contract`'s note on
...
`integral.candidate.CONSTRAINT_FIELD_NAMES`. Hardcoding "step-02" would go
...
from integral.candidate import CONSTRAINT_FIELD_NAMES, EmploymentModeName
...
def _main(argv: list[str]) -> int:
    """`python -m integral.employment_mode [--check] [--write-evidence [PATH]]`.
