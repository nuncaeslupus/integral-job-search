...
def measure(
    ...
    with tempfile.TemporaryDirectory(prefix="integral-roundtrip-") as scratch_dir:
    ...
    if refused:
        ...
        if pending:
            print(
                ...
                f"then import again:\n"
                f"  uv run python -m integral.suggestions propose {args.file}",
                file=sys.stderr,
            )
        return 2
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m integral.harness", description=__doc__)
    parser.add_argument("--store", default=str(DEFAULT_STORE_PATH))
    # Not `required=True`: a bare `python -m integral.harness` has to do
...
def _main(argv: list[str] | None = None) -> int:
    ...
    Named `_main` rather than `main` for the same reason `integral.suggestions`
