...
`integral.step_skills`' `steps_with_a_skill_fraction`, which is measured there
...
# `audit_library.py`'s constant, quoted rather than imported: since T58 that
# file lives in the installed `skill-workshop` plugin, outside this repository
# altogether, and importing across that boundary would make this module fail to
# load on a machine that has not installed it. The equality is asserted by test
# — which skips, rather than fails, where the plugin is not resolvable.
...
def configured_budget(config: Path = CONFIG_PATH) -> tuple[int, BudgetSource]:
    ...
    if not isinstance(value, int) or isinstance(value, bool) or value <= 0:
        raise SkillBudgetError(f"{config}: {CONFIG_KEY} must be a positive integer, got {value!r}")
    return value, "config"
...
def measure(
    ...
    from integral.step_skills import measure as measure_step_skills
...
def _main(argv: list[str]) -> int:
    """`python -m integral.skill_budget [--check] [--budget N]` → S10's gate.
