...
non-insistence rule (`integral.decline`). `resolve` consults `DeclineLedger`
...
through `integral.profile.rebuild`.** T6's `_build_constraints` folds any
...
**Open hazard, not this module's file to close.** `integral.revision.refresh`
calls `integral.profile.rebuild` for every derived artefact whenever an
...
from integral.profile import DERIVED_DIR, EvidenceLog, EvidenceRow
...
# §2 names this step "Constraints"; `integral.decline`'s subjects are
...
def write_constraints(
    ...
    `integral.revision`'s staleness detection keeps working over this file
...
def write_evidence(evidence: Path = DEFAULT_EVIDENCE_PATH) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="integral-t41-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m integral.constraints_step [path]` → T41's gate evidence."""
