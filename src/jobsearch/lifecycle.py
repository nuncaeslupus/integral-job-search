...
**Ownership boundary, stated once.** T11 (`jobsearch.offers`) owns the `Offer`
schema and `offers/<id>.json`'s file contract — `extra="forbid"`, so nothing
here adds a field to that file. T13 (`jobsearch.dedup`) owns the `Tombstone`
...
from jobsearch.dedup import Tombstone, load_tombstones, tombstone_match
from jobsearch.identity import IdentityError, ProfileStore, create_profile
from jobsearch.offers import Offer, OfferError, OfferStatus, Strict, connect_manual, load_offer
from jobsearch.offers import save_offer as _save_offer_body
...
def probe_lifecycle() -> dict[str, Any]:
    ...
    *only then* scans the tree for what survived. `resurrected_purged_offers`
    ...
    with tempfile.TemporaryDirectory(prefix="jobsearch-s5-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m jobsearch.lifecycle [path]` -> S5's gate evidence."""
