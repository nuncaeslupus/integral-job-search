...
`jobsearch.profile` folds whatever `dimensions` a constraint evidence row
...
# imported from `jobsearch.step_runtime.RESOLVED_STATES`: that module reads
...
class ConstraintField(Strict):
    ...
    def _shape_matches_state(self) -> ConstraintField:
        ...
        if self.state == "stated":
            ...
            if missing:
                raise ValueError(
                    f"{type(self).__name__}: state 'stated' requires "
...
class Relocation(ConstraintField):
    ...
    def _conditional_names_its_condition(self) -> Relocation:
        ...
        if self.willingness != "conditional" and (self.condition or self.confirmed_offers):
            raise ValueError(
                "Relocation: condition/confirmed_offers only apply to willingness "
                "'conditional'"
            )
...
class CandidateConstraints(Strict):
    ...
    def outstanding(self) -> tuple[str, ...]:
        ...
        """
        return tuple(
            name for name, value in self.as_dict().items() if value.state == "unknown"
        )
...
def _main(argv: list[str]) -> int:
    """`python -m jobsearch.candidate [path]` → T24's gate evidence."""
