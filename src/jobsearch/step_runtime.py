...
DETECTORS: dict[str, Detector] = {
    ...
    "claimed_facts": lambda view: view.has_file("cv", "master.json")
    or any(row.step == "intake" for row in view.rows),
    ...
    "traits": lambda view: bool(
        (view.json_at("profile", "traits.json") or {}).get("dimensions")
    ),
...
def present_artefacts(view: ProfileView, steps: StepList) -> set[str]:
    ...
    """
    present = {
        artefact for artefact, detector in DETECTORS.items() if detector(view)
    }
...
def missing_inputs(step: Step, present: Iterable[str]) -> tuple[str, ...]:
    ...
    return tuple(
        ...
        if not read.optional and read.artefact not in have
    )
...
def write_evidence(evidence: Path = DEFAULT_EVIDENCE_PATH) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="jobsearch-t34-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m jobsearch.step_runtime [path]` → T34's gate evidence."""
