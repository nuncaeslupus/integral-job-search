...
class DeclineLedger:
    ...
    def decline(self, subject: str, *, step: str, at: str) -> Entry:
        ...
        return self._append(
            Entry(subject=_validate(subject), action="declined", step=step, at=at)
        )
...
def write_evidence(evidence: Path = DEFAULT_EVIDENCE_PATH) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="jobsearch-t40-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m jobsearch.decline [path]` → T40's gate evidence."""
