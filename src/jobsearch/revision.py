...
from jobsearch.profile import EvidenceLog, EvidenceRow, ProfileRevision
...
def _manifest_revision(store: ProfileStore, relative: Path) -> ProfileRevision | None:
    ...
    from jobsearch.profile import DERIVED_MANIFEST
...
def refresh(store: ProfileStore) -> Refreshed:
    ...
    """
    from jobsearch.profile import rebuild
...
def write_evidence(evidence: Path = DEFAULT_EVIDENCE_PATH) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="jobsearch-t37-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m jobsearch.revision [path]` → T37's gate evidence."""
