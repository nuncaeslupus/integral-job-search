...
from jobsearch.revision import stale_artefacts
...
def write_evidence(evidence: Path = DEFAULT_EVIDENCE_PATH) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="jobsearch-t36-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m jobsearch.freshness [path]` → T36's gate evidence."""
