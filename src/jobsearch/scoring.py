...
from jobsearch.profile import EvidenceLog, EvidenceRow, ProfileRevision, rebuild
...
def score(store: ProfileStore, decision: Decision, *, at: str) -> dict[str, str]:
    ...
    if not decision.run or decision.trigger is None:
        raise ScoringError(
            f"refusing to score without one of §4.2's triggers ({decision.reason})"
        )
...
def write_evidence(evidence: Path = DEFAULT_EVIDENCE_PATH) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="jobsearch-t39-") as tmp:
...
def _main(argv: list[str]) -> int:
    """`python -m jobsearch.scoring [path]` → T39's gate evidence."""
