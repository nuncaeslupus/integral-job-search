    ...
    for step in required:
        for read in step.reads:
            if read.optional or read.artefact in reachable:
                continue
            producers = sorted(
                other.id for other in steps.steps if read.artefact in other.produces
            )
            if producers:
                violations.append(
...
def drift_violations(
    steps: StepList, spec_path: Path = DEFAULT_PROCESS_DOC
) -> list[str]:
...
def _main(argv: list[str]) -> int:
    """`python -m jobsearch.step_graph [path]` → T30's gate evidence."""
